...
export function ProjectMessages({ projectId, messages }: { projectId: string; messages: ProjectMessageItem[] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/projects/messages/create", {
      method: "POST",
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-white/[0.035] p-5">
      <p className="text-xs font-semibold uppercase tracking-[0.24em] text-cyberBlue">Client Communication</p>
      ...
      <form ref={formRef} onSubmit={submit} className="mt-4">
        <textarea name="message" required placeholder="Send an update to the client." className="min-h-24 w-full rounded-2xl border border-white/10 bg-ink/60 px-3 py-3 text-sm text-frost outline-none placeholder:text-mist/70" />
